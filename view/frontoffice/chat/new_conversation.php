...
// Determine counterpart role: freelancers chat with clients, clients chat with freelancers.
$myRole       = strtolower(trim((string)($currentUserRole ?? '')));
$isFreelancer = ($myRole === 'freelancer');
$counterpartRole       = $isFreelancer ? 'client'    : 'freelancer';
$counterpartLabelOne   = $isFreelancer ? 'client'    : 'freelancer';
$counterpartPlaceholder= $isFreelancer ? 'un client' : 'un freelancer';

$candidatesRaw = $isFreelancer ? $chatController->getClients() : $chatController->getFreelancers();
$candidates    = array_values(array_filter($candidatesRaw, fn($c) => (int)$c['id'] !== $currentUserId));

// Pré-remplissage depuis ?user2= (lien "Contacter" depuis l'index) — accepted seulement si dans la liste candidate
if (isset($_GET['user2']) && is_numeric($_GET['user2'])) {
    $hint = (int)$_GET['user2'];
    foreach ($candidates as $c) {
        if ((int)$c['id'] === $hint) { $user2_id = $hint; break; }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Sécurité serveur : refuser self-chat et hors-rôle
    $allowedIds = array_map(fn($c) => (int)$c['id'], $candidates);
    if ((int)$user2_id === $currentUserId) {
        $errors[] = "Vous ne pouvez pas démarrer une conversation avec vous-même.";
    } elseif (!in_array((int)$user2_id, $allowedIds, true)) {
        $errors[] = "Le " . $counterpartLabelOne . " sélectionné est invalide.";
    } else {
...
// Load current user's photo for the nav profile chip
...
<head>
    ...
    <title>Nouvelle Conversation — SkillBridge</title>
    ...
    <style>
      ...
      *, *::before, *::after { box-sizing: border-box; }
      ...
      .form-text { color: var(--ink-soft); font-size:.85rem; margin-top:6px; }
      .form-select { padding:14px 16px; font-size:1rem; }
...
<header class="sb-header">
  <div class="container">
    ...
    <nav class="sb-nav">
      ...
      <a href="new_conversation.php" class="active">Nouveau Chat</a>
    </nav>
      ...
      <div class="row justify-content-center">
        <div class="col-md-10 col-lg-8 col-xl-7">
          ...
          <div class="text-center mb-4">
            <span class="eyebrow honey"><span class="dot"></span> Nouveau chat</span>
            <h1 class="display-x mt-3 mb-2">Démarrer une <span class="accent">conversation</span>.</h1>
            <p class="lead-x mb-0">
              Choisissez <?= htmlspecialchars($counterpartPlaceholder) ?> dans la liste ci-dessous pour ouvrir un nouvel échange.
            </p>
          </div>
          ...
          <div class="auth-card">
            ...
            <form method="POST" action="new_conversation.php" novalidate>
              <div class="mb-4">
                <label for="user2_id" class="form-label">
                  Choisir <?= htmlspecialchars($counterpartPlaceholder) ?> <span style="color:#DC2626;">*</span>
                </label>
                <?php if (empty($candidates)): ?>
                  <div class="sb-alert" style="background:var(--bg); border:1px dashed var(--rule); color: var(--ink-mute);">
                    <i class="bi bi-info-circle"></i>
                    Aucun <?= htmlspecialchars($counterpartLabelOne) ?> disponible pour l'instant.
                  </div>
                <?php else: ?>
                  <select class="form-select" id="user2_id" name="user2_id">
                    <option value="">-- Sélectionnez <?= htmlspecialchars($counterpartPlaceholder) ?> --</option>
                    <?php foreach ($candidates as $cand): ?>
                      <option value="<?= htmlspecialchars($cand['id']) ?>" <?= ((int)$user2_id === (int)$cand['id']) ? 'selected' : '' ?>>
                        <?= htmlspecialchars($cand['prenom'] . ' ' . $cand['nom']) ?>
                      </option>
                    <?php endforeach; ?>
                  </select>
                  <div class="form-text">
                    <i class="bi bi-info-circle me-1"></i>Sélectionnez le <?= htmlspecialchars($counterpartLabelOne) ?> avec qui vous souhaitez discuter.
                  </div>
                <?php endif; ?>
              </div>
              ...
              <a href="conversations.php" class="btn-ghost">
                <i class="bi bi-x-lg"></i> <?= empty($candidates) ? 'Retour' : 'Annuler' ?>
              </a>
            </form>
