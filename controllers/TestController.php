...
class TestController {
    ...
    public function frontoffice() {
        ...
        $leaderboard   = $resultatModel->getLeaderboard(5)->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function generateAI() {
        ...
        // Log API error responses if we got a valid JSON but it's an error
        if (isset($response['error'])) {
            file_put_contents('ai_debug.txt', "\nAPI RETURNED ERROR: " . print_r($response['error'], true), FILE_APPEND);
            header("Location: index.php?action=index&error=api_error");
            exit();
        }
        ...
        if (isset($response['candidates'][0]['content']['parts'][0]['text'])) {
            ...
            // Nettoyer les blocs markdown et les espaces
            $cleanText = str_replace(['```json', '```', 'JSON', 'json'], '', $jsonText);
            $cleanText = trim($cleanText);
            
            // Essayer de trouver le tableau JSON avec une expression régulière (tout ce qui est entre crochets [])
            ...
            if (is_array($questions)) {
                ...
                file_put_contents('ai_debug.txt', "\nJSON PARSE FAILED. CLEAN TEXT WAS:\n" . $cleanText, FILE_APPEND);
                file_put_contents('ai_debug.txt', "\nJSON ERROR: " . json_last_error_msg(), FILE_APPEND);
            }
        } else {
            file_put_contents('ai_debug.txt', "\nNO CANDIDATES FOUND IN RESPONSE. RESPONSE WAS:\n" . print_r($response, true), FILE_APPEND);
        }
    ...
    public function submitTest() {
        ...
        $resultatModel->user_name = isset($_SESSION['user_nom']) ? $_SESSION['user_nom'] : 'Anonyme';
