...
class PropositionController
{
     ...
     */
    public function getAll(string $sort = 'recent', string $search = '', ?int $demandeId = null): array
    {
        try {
            ...
            if ($demandeId !== null) {
                $sql .= " AND p.demande_id = :demande_id";
    ...
    {
        ensure_propositions_user_id_column($this->pdo);
    ...
    {
        ensure_propositions_user_id_column($this->pdo);
        $sql = "INSERT INTO propositions (demande_id, freelancer_name, message, price, user_id, created_at)
                VALUES (:demande_id, :freelancer_name, :message, :price, :user_id, :created_at)";
        try {
            $query = $this->pdo->prepare($sql);
            $query->execute([
                ...
                ':user_id' => $proposition->getUser_id(),
