...
try {
    ...
    foreach ($propositions as &$proposition) {
        $proposition['ai_match'] = scoreProposalMatch($demande, $proposition);
    }
    unset($proposition);
    ...
    <?php else: ?>
      <?php foreach ($propositions as $p): ?>
        <div class="proposition-card">
          ...
          <?php if (!empty($p['ai_match'])): ?>
            <div class="alert-success-sb mt-3 mb-0">
              <i class="bi bi-stars"></i>
              <span><?= htmlspecialchars($p['ai_match']) ?></span>
            </div>
          <?php endif; ?>
