...
$propositions = [];
try {
    $propositions = $controller->getAll();
    $demandeOptions = $controller->getDemandesOptions();
    if ($demandeFilterId !== null) {
        $propositions = array_values(array_filter($propositions, function ($proposition) use ($demandeFilterId) {
            return (int) ($proposition['demande_id'] ?? 0) === $demandeFilterId;
        }));
    }
    if ($editingId !== null) {
